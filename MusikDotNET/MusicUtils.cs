...
namespace MusikDotNET
{
    public static class MusicUtils
    {
        ...
        public static string[] modifiers = new[] { "7/4", "7/5", "7+", "7", "6", "5+", "5", "4", "dim" };
        ...
        public static void FlatToSharp(ref Note note, NoteLayout layout)
        {
            switch (layout)
            {
                case NoteLayout.Italian:
                    {
                        if (note.Name.Contains("RE") && note.Name.Contains("b"))
                            note.Name = "DO#" + note.Name.Substring(2).Replace("b", "");
                        else if (note.Name.Contains("MI") && note.Name.Contains("b"))
                            note.Name = "RE#" + note.Name.Substring(2).Replace("b", "");
                        else if (note.Name.Contains("SOL") && note.Name.Contains("b"))
                            note.Name = "FA#" + note.Name.Substring(3).Replace("b", "");
                        else if (note.Name.Contains("LA") && note.Name.Contains("b"))
                            note.Name = "SOL#" + note.Name.Substring(2).Replace("b", "");
                        else if (note.Name.Contains("SI") && note.Name.Contains("b"))
                            note.Name = "LA#" + note.Name.Substring(2).Replace("b", "");
                    }
                    break;
                case NoteLayout.English:
                    {
                        if (note.Name.Contains("D") && note.Name.Contains("b"))
                            note.Name = "C#" + note.Name.Substring(2).Replace("b", "");
                        else if (note.Name.Contains("E") && note.Name.Contains("b"))
                            note.Name = "D#" + note.Name.Substring(2).Replace("b", "");
                        else if (note.Name.Contains("G") && note.Name.Contains("b"))
                            note.Name = "F#" + note.Name.Substring(3).Replace("b", "");
                        else if (note.Name.Contains("A") && note.Name.Contains("b"))
                            note.Name = "G#" + note.Name.Substring(2).Replace("b", "");
                        else if (note.Name.Contains("B") && note.Name.Contains("b"))
                            note.Name = "A#" + note.Name.Substring(2).Replace("b", "");
                    }
                    break;
            }
        }

        public static void Simplify(ref Note note)
        {
            foreach (string mod in modifiers)
                if (note.Name.Contains(mod))
                {
                    note.Name = note.Name.Replace(mod, "");
                    break;
                }
        }
